...
class PlayTaskSoundActivity : AppCompatActivity(), View.OnClickListener {
    ...
    inner class PlaybackListener : PlaybackInfoListener() {
        ...
        override fun onPositionChanged(position: Int) {
            if (!mUserIsSeeking) {
                val milliSec80Per = (mPlayerAdapter!!.mediaPlayer.duration.toLong() * 90) / 100
                if (!isAPICallingDone) {
...
//                Utils.callUpdateSeconds("", jsonObj)
    ...
    private fun callRewardsAPI(momentId: String) {
        ...
        val call = ApiUtils.getAPIService().getRewardsOnPlay(userId, "sound", momentId)
...
//                    Utils.callUpdateSeconds("", jsonObj)
    ...
    override fun onBackPressed() {
        ...
        if (isTaskRoot) {
            val intent = Intent(this@PlayTaskSoundActivity, MainActivity::class.java)
            startActivity(intent)
        }
    }
