...
class VerifyedOTPActivity : AppCompatActivity(), View.OnClickListener {
                    ...
                    if (otp.length > 3) {
                        if (checkInternetConnection()) {
                            ...
                            if (otp.equals(strotp)) {
                                hideKeyBoard()
                                verifyOTP(strEmail.toString(), otp)
                            } else {
                                Toast.makeText(
                                    this@VerifyedOTPActivity, "Please Enter Correct OTP",
                                    Toast.LENGTH_SHORT
                                ).show()
                            }
...
//
//        val call = ApiUtils.getAPIService().verfiyOTP(pCustomerEmail, otp)
//
